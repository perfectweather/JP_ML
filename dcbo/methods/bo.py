...
class BO(Root):
    ...
    def _update_bo_model(
        ...
        if not self.bo_model[temporal_index][exploration_set]:
            ...
            y = self.interventional_data_y[temporal_index][exploration_set].squeeze(1)
            ...
            bo_kernel = tfp.math.psd_kernels.ExponentiatedQuadratic(amplitude=variance, length_scale=1.0)
            ...
            self.bo_model[temporal_index][exploration_set].set_data(
                X=tf.convert_to_tensor(self.interventional_data_x[temporal_index][exploration_set], dtype=tf.float64),
                Y=tf.convert_to_tensor(self.interventional_data_y[temporal_index][exploration_set].squeeze(1), dtype=tf.float64),
            )
