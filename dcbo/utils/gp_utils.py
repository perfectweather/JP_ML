...
def fit_gp(x, y, lengthscale=1.0, variance=1.0, noise_var=1.0, ard=False, n_restart=10, seed: int = 0,) -> Callable:
    ...
    # kernel = tfp.math.psd_kernels.ExponentiatedQuadratic(amplitude=variance, length_scale=lengthscale)

    # # model = GPRegression(X=x, Y=y, kernel=kernel, noise_var=noise_var)
    # observation_noise_variance = tf.constant(noise_var, dtype=tf.float64)
    ...
    #     gp.kernel.amplitude.assign(params[0])
    #     gp.kernel.length_scale.assign(params[1])
    #     gp.observation_noise_variance.assign(params[2])
    ...
    #     # 随机初始化参数
    #     initial_amplitude = np.random.gamma(2.0, 1.0 / 0.1)
    #     initial_length_scale = np.random.gamma(2.0, 1.0 / 0.1)
    #     initial_noise_variance = np.random.gamma(2.0, 1.0 / 0.1)

    #     initial_params = tf.convert_to_tensor(
    #         [initial_amplitude, initial_length_scale, initial_noise_variance],
    #         dtype=tf.float64
    #     )
    ...
    #         final_params = results.position
    #         final_nll = neg_log_likelihood(final_params).numpy()
